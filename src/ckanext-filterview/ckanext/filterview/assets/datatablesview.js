...
function initFilterObserver () {
  ...
  console.log('initFilterObserver called')
  const infoEl = document.getElementById('dtprv_info')
  if (!infoEl) return
  const filterObserver = new MutationObserver(function (e) {
    const info = document.getElementById('dtprv_info')
    const icon = document.getElementById('filterinfoicon')
    if (!info || !icon) return
  ...
  try {
    filterObserver.observe(infoEl, { characterData: true, subtree: true, childList: true })
  } catch (e) {}
  
  // bind filter change events using event delegation
  console.log('Setting up filter event listeners')
  $('#dtprv').off('keyup.dtfilter change.dtfilter', 'thead .dt-col-op, thead .dt-filter-val')
  $('#dtprv').on('keyup.dtfilter change.dtfilter', 'thead .dt-col-op, thead .dt-filter-val', function () {
    console.log('Filter control changed:', $(this).attr('class'), 'value:', $(this).val())
    if (typeof datatable !== 'undefined' && datatable) {
      console.log('Triggering datatable.draw()')
...
this.ckan.module('datatables_view', function (jQuery) {
  ...
  return {
    initialize: function () {
      ...
      const clientSideFiltering = (dtprv.data('client-side-filtering') === true ||
        String(dtprv.data('client-side-filtering')).toLowerCase() === 'true')
      const ajaxurlAll = dtprv.data('ajaxurl-all') || ''
      const clientSideMaxRows = parseInt(dtprv.data('client-side-max-rows'), 10) || 10000
      ...
      gdataDict.forEach((colDefn, idx) => {
        ...
        switch (colDefn.type) {
          ...
          case 'timestamptz':
            colDict.type = 'date'
            if (formatdateflag) {
              const fromFmt = window.moment.ISO_8601
              const toFmt = dateformat
              const loc = languagecode
              colDict.render = function (d, type, row) {
                if (d == null || d === '') {
                  return type === 'sort' || type === 'type' ? 0 : d
                }
                try {
                  const m = window.moment(d, fromFmt, loc, true)
                  if (!m.isValid()) {
                    return type === 'sort' || type === 'type' ? 0 : d
                  }
                  return m.format(type === 'sort' || type === 'type' ? 'x' : toFmt)
                } catch (e) {
                  return type === 'sort' || type === 'type' ? 0 : d
                }
              }
      ...
      // create column filters (server-side: single input; client-side: operator + value for num/date)
      console.log('Creating column filters. clientSideFiltering =', clientSideFiltering)
      $('.fhead').each(function (i) {
        const thecol = this
        const colname = thecol.textContent.trim()
        const colid = 'dtcol-' + validateId(colname) + '-' + i
        const coltype = ($(thecol).data('type') || '').toString()
        const isNumericOrDate = coltype === 'numeric' || coltype.indexOf('timestamp') === 0
        const placeholderText = formatdateflag && coltype.substr(0, 9) === 'timestamp' ? ' placeholder="yyyy-mm-dd"' : ''

        console.log(`Column ${i}: name="${colname}", type="${coltype}", isNumericOrDate=${isNumericOrDate}`)

        if (clientSideFiltering && isNumericOrDate) {
          const wrap = $('<div class="dt-filter-cell"></div>')
          const opSelect = $('<select class="dt-col-op form-control input-sm" data-colidx="' + i + '" data-colname="' + esc(colname) + '" data-coltype="' + esc(coltype) + '"></select>')
          const opOptions = [
            { v: '', t: that._('(any)') },
            { v: 'eq', t: '=' },
            { v: 'ne', t: '!=' },
            ...
            { v: 'lt', t: '<' },
            { v: 'lte', t: '<=' },
            { v: 'between', t: that._('between') },
            { v: 'empty', t: that._('is empty') },
            { v: 'nempty', t: that._('is not empty') }
          ]
          opOptions.forEach(function (o) {
            opSelect.append($('<option value="' + o.v + '">' + o.t + '</option>'))
          })
          const valInput = $('<input class="dt-filter-val fhead form-control input-sm" type="text" autocomplete="on" data-colname="' + esc(colname) + '"' + placeholderText + ' placeholder="' + (coltype.indexOf('timestamp') === 0 ? 'yyyy-mm-dd or min,max' : '') + '"/>')
          wrap.append(opSelect).append(valInput)
          $(thecol).empty().append(wrap)
          
          // Bind events directly to the created elements
          console.log('Binding events for numeric/date column:', colname)
          opSelect.on('change', function () {
            console.log('Operator changed for', colname, ':', $(this).val())
            if (typeof datatable !== 'undefined' && datatable) datatable.draw(false)
          })
          valInput.on('keyup change', function () {
            console.log('Value changed for', colname, ':', $(this).val())
            if (typeof datatable !== 'undefined' && datatable) datatable.draw(false)
          })
        } else {
          $('<input id="' + colid + '" name="' + colid + '" autosave="' + colid + '"' +
              placeholderText +
              ' class="fhead form-control input-sm' + (clientSideFiltering ? ' dt-filter-val" data-colname="' + esc(colname) + '"' : '"') +
              ' type="search" results="10" autocomplete="on" style="width:100%"/>')
            .appendTo($(thecol).empty())
            .on('keyup search', function (event) {
              if (clientSideFiltering) {
                console.log('Text filter keyup for', colname, ':', this.value)
                if (typeof datatable !== 'undefined') datatable.draw(false)
                return
              }
              const colSelector = colname + ':name'
              if (gisFirefox && event.keyCode === 27) this.value = ''
              if (event.keyCode === 13 || (this.value === '' && datatable.column(colSelector).search() !== '')) {
                datatable.column(colSelector).search(this.value).page(0).draw(false)
                gsearchMode = 'column'
              }
            })
        }
      ...
      // bind operator/value change to draw when client-side (datatable set later)
      if (clientSideFiltering) {
        $(document).on('change keyup', '#dtprv .dt-col-op, #dtprv .dt-filter-val', function () {
          console.log('Filter input changed, triggering redraw')
          if (typeof datatable !== 'undefined') {
            datatable.draw(false)
          } else {
            console.warn('Datatable not yet initialized')
          }
        })
      }
      ...
      datatable = $('#dtprv').DataTable({
        paging: true,
        serverSide: !clientSideFiltering,
        processing: true,
        ...
        columns: dynamicCols,
        ajax: {
          url: clientSideFiltering
            ? (ajaxurlAll + (ckanfilters ? '?filters=' + encodeURIComponent(ckanfilters) : ''))
            : ajaxurl,
          type: 'POST',
          timeout: 60000,
          headers: {
        ...
        initComplete: function (settings, json) {
          ...
          $('#filterinfoicon').dblclick(function () {
            ...
            $('th.fhead input').val('')
            if (clientSideFiltering) {
              $('th.fhead .dt-col-op').val('')
              $('th.fhead .dt-filter-val').val('')
            }
          })
      ...
      // client-side comparison filter: custom search per row
      if (clientSideFiltering) {
        console.log('Registering client-side filter function')
        $.fn.dataTable.ext.search.push(function (settings, searchData, dataIndex, rowData) {
          if (settings.nTable && settings.nTable.id !== 'dtprv') return true
          
          console.log('Custom filter executing for row:', dataIndex)
          
          for (let ci = 0; ci < gdataDict.length; ci++) {
            const colDef = gdataDict[ci]
            const colname = colDef.id
            const coltype = (colDef.type || '').toString()
            
            // 使用 data-colname 属性选择器直接查找筛选控件
            const $op = $('.dt-col-op[data-colname="' + colname + '"]')
            const $val = $('.dt-filter-val[data-colname="' + colname + '"]')
            
            const op = $op.length ? $op.val() : ''
            let val = $val.length ? ($val.val() || '').trim() : ''
            
            if (!op && !val) continue
            
            console.log(`Filter check - Column: ${colname}, Type: ${coltype}, Op: ${op}, Val: ${val}, Row: ${dataIndex}`)
            ...
            if (op === 'nempty') {
              console.log(`  -> Not empty check: ${!isEmpty}`)
              if (isEmpty) return false
              continue
            }
            if (!val && op !== 'empty' && op !== 'nempty') continue
            const isNumeric = coltype === 'numeric' || coltype === 'int' || coltype === 'float'
            const isDate = coltype.indexOf('timestamp') === 0 || coltype === 'date'
            let cellVal
            let compareVal
            if (isNumeric) {
              cellVal = parseFloat(String(raw).replace(/,/g, ''))
              compareVal = parseFloat(String(val).replace(/,/g, ''))
              if (isNaN(cellVal)) cellVal = null
              if (isNaN(compareVal)) continue
              console.log(`  -> Numeric comparison: cellVal=${cellVal}, compareVal=${compareVal}`)
            } else if (isDate) {
              cellVal = raw ? new Date(String(raw)).getTime() : null
              if (val.indexOf(',') !== -1) {
                const parts = val.split(',').map(function (s) { return s.trim() })
                const minV = parts[0] ? new Date(parts[0]).getTime() : null
                const maxV = parts[1] ? new Date(parts[1]).getTime() : null
                if (op === 'between' && minV !== null && maxV !== null) {
                  if (cellVal === null) return false
                  const result = cellVal >= minV && cellVal <= maxV
              ...
              cellVal = String(raw || '')
              compareVal = String(val)
            }
            if (cellVal === null && op !== 'eq' && op !== 'ne') {
              console.log(`  -> Null value, returning false`)
              return false
            }
            let result
            switch (op) {
              case 'eq':
              case '=': 
                result = isNumeric || isDate ? cellVal === compareVal : cellVal.toLowerCase().indexOf(compareVal.toLowerCase()) !== -1
              ...
              case '<=': 
                result = cellVal <= compareVal
                console.log(`  -> lte: ${result}`)
                if (!result) return false
                break
              case 'between':
                if (isNumeric && val.indexOf(',') !== -1) {
                  const parts = val.split(',').map(function (s) { return parseFloat(s.trim()) })
                  if (parts.length >= 2 && !isNaN(parts[0]) && !isNaN(parts[1])) {
                    result = cellVal !== null && cellVal >= parts[0] && cellVal <= parts[1]
                    console.log(`  -> between: ${result}`)
                    if (!result) return false
                  }
                }
                break
              default:
                if (!op && val) {
                  const s1 = String(cellVal != null ? cellVal : '').toLowerCase()
                  const s2 = String(compareVal != null ? compareVal : '').toLowerCase()
                  result = s1.indexOf(s2) !== -1
                  console.log(`  -> default contains: ${result}`)
                  if (!result) return false
                }
            }
          }
        ...
        console.log('Client-side filter function registered')
      }
      ...
      datatable.on('draw.dt', function() {
        const json = datatable.ajax && datatable.ajax.json ? datatable.ajax.json() : null
        if (!json || !json.total_was_estimated) return
        const infoEl = document.getElementById('dtprv_info')
        if (!infoEl) return
        infoEl.innerHTML += ' ' + that._('(estimated)')
      })
      ...
      datatable.on('i18n', function () {
        ...
        setTimeout(function() { initFilterObserver() }, 100)
      })
