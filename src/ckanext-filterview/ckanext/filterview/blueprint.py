...
def ajax(resource_view_id: str):
    ...
    # first search: count total records
    ...
    # second search: get filtered records
    try:
...
def ajax_all(resource_view_id: str):
    u'''
    Return all records (up to client_side_max_rows) for client-side filtering.
    Used when resource view has client_side_filtering enabled.
    '''
    resource_view = get_action(u'resource_view_show')({}, {u'id': resource_view_id})
    view_filters = resource_view.get(u'filters', {})
    user_filters = decode_view_request_filters()
    filters = merge_filters(view_filters, user_filters)
    max_rows = int(resource_view.get(u'client_side_max_rows') or 10000)

    datastore_search = get_action(u'datastore_search')
    try:
        count_response = datastore_search(
            {}, {
                u'resource_id': resource_view[u'resource_id'],
                u'limit': 0,
                u'filters': view_filters,
                u'total_estimation_threshold': ESTIMATION_THRESHOLD,
            }
        )
    except ObjectNotFound:
        return json.dumps({'error': 'Object not found'}), 404
    ...
    total = count_response.get(u'total', 0)
    limit = min(total, max_rows)
    cols = [f[u'id'] for f in count_response[u'fields']]
    if u'show_fields' in resource_view:
        cols = [c for c in cols if c in resource_view[u'show_fields']]

    # datasearch at one time
    try:
        response = datastore_search(
            {}, {
                u'resource_id': resource_view[u'resource_id'],
        ...
        return json.dumps({'error': 'Datastore search failed'}), 400
    ...
    for row in response[u'records']:
        record = {
            colname: escape(
                str(null_label if row.get(colname, u'') is None else row.get(colname, u''))
            )
            for colname in cols
        }
        record['DT_RowId'] = 'row' + str(row.get(u'_id', u''))
    ...
    dtdata = {
        u'data': data,
        u'recordsTotal': total,
        u'recordsFiltered': total,
    }
    return json.dumps(dtdata), 200
...
datatablesview.add_url_rule(
    u'/datatables/ajax-all/<resource_view_id>',
    view_func=ajax_all,
