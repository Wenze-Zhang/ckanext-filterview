...
def test_ajax_all_returns_full_data(app, user):
    ...
    view = factories.ResourceView(
        ...
        client_side_max_rows=10000,
    )
    resp = app.post(
        url_for('datatablesview.ajax_all', resource_view_id=view["id"]),
        data={},
    )
    assert resp.status_code == 200
    data = json.loads(b''.join(resp.response).decode('utf-8'))
    assert 'data' in data
...
def test_ajax_all_respects_max_rows(app, user):
    ...
    assert data['recordsFiltered'] == 10
